...
typedef struct {
  int nFil;
nFil = 0   # deslplazamiento para llegar a este campo en la estructura
  int nCol;
nCol = 4
  float elementos[];
elementos = 0
...
sizeF = 4   # Numero de bytes en un float
LF = 10    # Caracter salto de linea
...
# mat -> $a0 -> $s0    Despues colocar esto bien
# nFil -> $s1        Hay que meter todo esto en la pila
# nCol -> $s2
# elem -> $s3
...
# c -> $s5
# $f12 = elem[f * nCol + c]
       mul  $t0,$s4,$s2   # f * numCol
       addu $t0,$t0,$s5  # f * numCol + c
       mul $t0,$t0,sizeF  # (f * numCol + c) * tamElem
  ...
  while(true) {
    ...
    if(opcion == 3 || opcion == 4) {
      ...
      if ((indCol < 0) || (indCol >= matTrabajo->nCol)){
           lw $t2,4($s0)    # 4 es el desplazamiento necesario en la estructura par llegar a la variable nCol
