...
{ 
  ...
  if (!(i > 0 && i <= matrix_.get_m() && j > 0 && j <= matrix_.get_n())) {
    return false;
  } else if (matrix_(i, j) == WALL_ID || visited_(i, j) == true) {
    return false;
  } else {
    return true;
  }
...
for (int direccion{0}; direccion < 4; direccion++) {
  int i_actual {i + i_d[direccion]};
  int j_actual {j + j_d[direccion]};
  if (is_ok_(i_actual, j_actual)) {
    if (solve_(i_actual, j_actual)) {
      matrix_(i_actual, j_actual) = PATH_ID;
      celda celda_temp{i_actual, j_actual}; 
      celdas_solucion.push(celda_temp);
      return true;
    }
  } 
}
...
void maze_t::MostrarPosicionesCamino() {
  bool first = true;
  std::cout << "Posiciones de las celdas recorridas desde el principio hasta la salida: "; 
  while (!celdas_solucion.empty()) {
    celda aux = celdas_solucion.top();
    celdas_solucion.pop();
    if (!first) {
      std::cout << ", ";
    }
    std::cout << '(' << aux.posicion_i << ", " << aux.posicion_j << ')';
    first = false;
  }
  std::cout << std::endl;
}
