...
template<class T> void stack_l_t<T>::IntercambiarPrimeros() {
  assert(top() != NULL);
  T temp = top();
  pop();
  assert(top() != NULL);
 ...
 template<class T> void stack_l_t<T>::EliminarImpares() {
   T temp;
   stack_l_t<T> pila_auxiliar;
   while (!empty() != NULL) {
     temp = top();
