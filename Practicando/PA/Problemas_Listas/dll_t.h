...
template <class T> class dll_t {
 public:
  ...
  void InsertarTrasNodo(dll_node_t<T>* nuevo_nodo, dll_node_t<T>* nodo_anterior);
  void DuplicarLista();
...
template <class T> void dll_t<T>::DuplicarLista() {
  sz_ = sz_ * 2;
}

template <class T> void dll_t<T>::InsertarTrasNodo(dll_node_t<T>* nuevo_nodo, dll_node_t<T>* nodo_anterior) {
  ++sz_;
  nodo_anterior -> get_next() -> set_prev(nuevo_nodo);
  // std::cout << "1 ";
  nuevo_nodo -> set_next(nodo_anterior -> get_next());
  // std::cout << "1 ";
  nuevo_nodo -> set_prev(nodo_anterior);
  // std::cout << "1 ";
  nodo_anterior -> set_next(nuevo_nodo);
}
